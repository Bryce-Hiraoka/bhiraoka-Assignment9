...
def update(frame, mlp, ax_input, ax_hidden, ax_gradient, X, y, x_min, x_max, y_min, y_max, z_min, z_max):
    ...
    for neuron in hidden:
        ...
        ax_gradient.add_patch(circle)
    circle = Circle(output, 0.1, color='salmon')
    ax_gradient.add_patch(circle)
    ...
    linewidth = 5  # Prevents edges from becoming too thick

    for i, input in enumerate(input):
        for j, neuron in enumerate(hidden):
            grad = mlp.dW1[i, j]
            linewidth = np.clip(np.abs(grad) / max_grad * linewidth, 0.1, linewidth)
            ax_gradient.plot([input[0], neuron[0]],
                             [input[1], neuron[1]],
                             color='gray', linewidth=linewidth)
    # Visual for hidden to output 
    for j, neuron in enumerate(hidden):
        grad = mlp.dW2[j, 0]
        linewidth = np.clip(np.abs(grad) / max_grad * linewidth, 0.1, linewidth)
        ax_gradient.plot([neuron[0], output[0]],
                         [neuron[1], output[1]],
                         color='gray', linewidth=linewidth)
